...
namespace PF___3.Array_Manipulator___Extended
{
    class Program
    {
            ...
            while (true)
            {
                ...
                switch (CommandLine[0])
                {
                    ...
                    case "print":
                        ...
                        break;
                    case "addrange":
                        AddRangeOfElementsToTheEnd(CommandLine, InputNumberList);
                        break;
                    case "insertset":
                        InsertsCollectionToIndex(CommandLine, InputNumberList);
                        break;
                    default:
        ...
        private static void InsertsCollectionToIndex(string[] CommandLine, List<int> InputNumberList)
        {
            int Index = int.Parse(CommandLine[2]);
            int Numbers = CommandLine.Length - 2;
            int[] Temp = new int[Numbers];
            for (int i = 0; i < Numbers; i++)
            {
                int temp = int.Parse(CommandLine[2+ i]);
                Temp[i] = temp;
            }
            InputNumberList.InsertRange(Index,Temp);
        }
        private static void AddRangeOfElementsToTheEnd(string[] CommandLine, List<int> InputNumberList)
        {
            int Numbers = CommandLine.Length - 1;
            int[] Temp = new int[Numbers];
            for (int i = 0; i < Numbers; i++)
            {
                int temp = int.Parse(CommandLine[1+i]);
                Temp[i] = temp;
            }
            InputNumberList.AddRange(Temp);
        }
        private static void SumListElementsByPairs(List<int> InputNumberList)
        {
            ...
            {
                int A = InputNumberList[i];
                int B = InputNumberList[i + 1];
                int PairSum = A + B;
                TempList.Add(PairSum);
            }
        ...
        {
            if (InputNumberList.Contains(Element))
            {
